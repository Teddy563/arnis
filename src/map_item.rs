...
/// Populated-chunk block bounds of a saved world, read straight from the region headers
/// (each `r.X.Z.mca` starts with 1024 location entries; a non-zero entry marks a stored chunk).
/// Returns `(min_x, min_z, max_x, max_z)` in block coords, or `None` for an empty world.
///
/// Used by the post-merge `--map-item-only` pass: Meld assembles many per-cell worlds into one
/// master world, so there is no single generation `XZBBox` to reuse. Scanning the merged regions
/// yields bounds that match exactly what is on disk, with no OSM re-parse and no seam risk.
pub fn world_block_bounds(world_path: &Path) -> Option<(i32, i32, i32, i32)> {
    let region_dir = world_path.join("region");
    let mut bounds: Option<(i32, i32, i32, i32)> = None;
    for entry in std::fs::read_dir(&region_dir).ok()? {
        let path = match entry {
            Ok(e) => e.path(),
            Err(_) => continue,
        };
        let name = match path.file_name().and_then(|s| s.to_str()) {
            Some(n) if n.starts_with("r.") && n.ends_with(".mca") => n,
            _ => continue,
        };
        // r.<X>.<Z>.mca
        ...
        // Read only the 4096-byte location header.
        ...
        for i in 0..1024usize {
            let off = i * 4;
            // 3-byte offset + 1-byte sector count; any non-zero means the chunk is stored.
            if header[off] == 0 && header[off + 1] == 0 && header[off + 2] == 0 {
                continue;
            }
            let cx = rx * 32 + (i % 32) as i32;
            let cz = rz * 32 + (i / 32) as i32;
            let (bx0, bz0) = (cx * 16, cz * 16);
            let (bx1, bz1) = (bx0 + 15, bz0 + 15);
            bounds = Some(match bounds {
                None => (bx0, bz0, bx1, bz1),
                Some((mnx, mnz, mxx, mxz)) => {
                    (mnx.min(bx0), mnz.min(bz0), mxx.max(bx1), mxz.max(bz1))
                }
            });
        }
    }
...
/// Renders + inserts the map for a world already on disk, deriving its footprint from the
/// saved regions. Entry point for the `--map-item-only` post-merge pass.
pub fn write_map_item_for_existing_world(world_path: &Path) -> Result<(), String> {
    let (min_x, min_z, max_x, max_z) =
        world_block_bounds(world_path).ok_or("world has no saved regions")?;
    let xzbbox = XZBBox::rect_from_min_max(min_x, min_z, max_x, max_z)?;
    write_map_item(world_path, &xzbbox)
}

// Renders the world map into a locked map covering the entire world and hands it to the player.
pub fn write_map_item(world_path: &Path, xzbbox: &XZBBox) -> Result<(), String> {
    ...
    // Render straight from the saved region files: 1 block = 1 pixel, origin at the bbox min.
    let img = crate::map_renderer::render_world_map_image(
        world_path,
        xzbbox.min_x(),
        xzbbox.max_x(),
        xzbbox.min_z(),
...
mod tests {
    ...
    fn writes_map_files_and_inventory_item() {
        ...
        write_map_item(&world, &xzbbox).unwrap();
