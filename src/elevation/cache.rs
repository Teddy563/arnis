...
/// Resolve the tile-cache root. ARNIS_CACHE_ROOT (set by Meld) wins, so every cache can live
/// in one project-local folder shared across worlds; otherwise the OS-standard cache directory
/// (AppData/Local on Windows, ~/.cache on Linux). Falls back to ./arnis-tile-cache.
fn tile_cache_base() -> PathBuf {
    if let Some(root) = std::env::var_os("ARNIS_CACHE_ROOT") {
        if !root.is_empty() {
            return PathBuf::from(root).join(TILE_CACHE_DIR_NAME);
        }
    }
...
pub fn get_cache_dir(provider_name: &str) -> PathBuf {
    tile_cache_base().join(provider_name)
}
...
pub fn cleanup_old_cached_files() {
    ...
    // Throttle: this walk is O(number of cached files) and ran on EVERY process spawn.
    // Meld's --download-terrain-only prewarm fills the cache with 500k+ tiles, so an
    // unthrottled sweep cost 1-18s per spawn for zero benefit (the tiles are fresh, nothing
    // ages out). Skip if we already swept within the last day; the first spawn past that
    // claims the window (writes the stamp) so concurrent spawns skip it.
