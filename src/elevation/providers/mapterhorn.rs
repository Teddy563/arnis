...
fn download_tile(
    ...
    // Offline mode: a cache MISS must not hit the network. Cached tiles are served
    // by fetch_tile before ever reaching here, so failing fast just turns an
    // uncached cell into flat/NaN ground via Arnis's existing fallback (matches the
    // AWS Terrain / regional-provider offline behaviour). Mapterhorn upstream has no
    // offline gate; this is the fork's --offline support.
    if std::env::var_os("ARNIS_OFFLINE").is_some() {
        return TileFetch::Failed(format!(
            "offline: Mapterhorn tile z{}_x{}_y{} not cached and download is disabled",
            key.z, key.x, key.y
        ));
    }
    ...
    for attempt in 0..TILE_DOWNLOAD_MAX_RETRIES {
        ...
        // Acquired per attempt, after any backoff sleep (upstream #1274 discipline).
