...
/// Meld always does). Lets cross-tile-sensitive code (e.g. per-chunk biome
/// latitude) switch to a position-only, cell-independent formula so adjacent cells
/// agree at their shared border. Single-world runs (no `--seed`) return false and
