...
use crate::element_processing::waterways::{
    is_channel_waterway, is_subgrade, waterway_width, MAX_WATERWAY_WIDTH,
};
...
pub fn apply_osm_land_override(
    ...
    // The fork's LandCoverData carries no cells_per_meter; derive it from the
    // grid-to-world factor. compute_grid_dims lays out world blocks = metres x
    // scale on both origin paths, so cells/metre = cells/block x blocks/metre.
    let band = band_cells(map.sx * scale);
        ...
        match elem {
            ProcessedElement::Way(way) => {
                ...
                if is_water_area_way(way) {
                    ...
                    if is_channel_waterway(kind) && !is_subgrade(way) {
                        let half = (waterway_width(way) / 2).max(0);
    ...
    fn way(id: u64, nodes: Vec<ProcessedNode>, tags: &[(&str, &str)]) -> ProcessedElement {
        ProcessedElement::Way(ProcessedWay {
            ...
            tags: tags
                ...
                .collect(),
            // fork-only fields, see osm_parser.rs
