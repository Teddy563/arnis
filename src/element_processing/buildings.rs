...
impl BuildingCategory {
    ...
    /// Which glass-family variant a self-declared glass tower becomes, so tagged glass
    /// buildings still vary between plain curtain, concrete grid, and corner-pillar looks.
...
impl BuildingStylePreset {
    ...
    /// Preset for glass-curtain towers with concrete corner pillars.
...
fn calculate_start_y_offset(
    ...
    if args.terrain() {
        ...
        //
        // Points are kept in WORLD coords; `editor.terrain_level` subtracts
        // the ground origin internally (correct in tile editors, unlike
        // get_min_coords), so we must NOT pre-subtract it here.
        let sample_points: Vec<(i32, i32)> =
