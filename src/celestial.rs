...
mod tests {
    ...
    /// Upstream expresses this as `world_scale() < OBJECT_SKIP_SCALE`, a constant this
    /// fork deliberately does not define (see the MELD-DIVERGENCE note in args.rs: Meld
    /// renders countries at 1:10 and needs its objects). Moon and Mars carry no OSM data
    /// and apply_body_defaults forces terrain-only anyway, so the claim worth holding here
    /// is that their fixed scale is genuinely the coarse one.
    #[test]
    fn fixed_scales_are_far_coarser_than_any_earth_project() {
        for body in [CelestialBody::Moon, CelestialBody::Mars] {
            assert!(
                body.world_scale() < 0.1,
                "{body:?} scale {} is not a coarse fixed scale",
                body.world_scale()
            );
        }
    }
