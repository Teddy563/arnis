...
impl SectionToModify {
    ...
    pub fn to_section(&self, y: i8) -> Section {
        ...
        if self.properties.is_empty() && !matches!(self.storage, BlockStorage::Uniform(_)) {
            // Build palette from unique blocks using a small id-indexed array.
            // Sized to BLOCK_ID_CEILING (not 256): our fork's Block id is u16 and
            // Meld blocks use ids 256.., which would overflow upstream's 256 array.
            // Iterate logical Blocks so both Full (u8) and FullWide sections get this path.
            let mut block_to_palette = [u16::MAX; crate::block_definitions::BLOCK_ID_CEILING];
        ...
        // Memo from the *identity* of a property compound to its palette slot, in front of
        // the content-keyed map below. The content map stays authoritative, so two distinct
        // Arcs holding equal NBT still collapse to one palette entry exactly as before - this
        // only avoids re-deriving the answer for an Arc already seen in this section.
        //
        // Without it, the debug-format below runs once per CELL. Producers that share one
        // interned Arc across a whole field (crops) therefore paid 4096 string formats and
        // allocations per section to rediscover the same eight compounds, which is most of
        // the save-time cost of a farmland-heavy world.
            ...
            let palette_index = match properties {
                Some(arc) => {
                    ...
                    if let Some(&idx) = ptr_lookup.get(&ptr_key) {
                        ...
                        // First sighting of this exact Arc in this section: fall back to the
                        // content key, so an equal compound from a different allocation still
                        // lands in the slot that already exists for it.
...
mod tests {
    ...
    /// The palette is keyed by property CONTENT, and the Arc-identity memo added in front
    /// of it must not change that. Two equal compounds arriving as separate allocations
    /// still have to collapse into one palette entry, or a world would gain duplicate
    /// palette slots (and a different block_hash) purely from how its producer allocated.
    #[test]
    fn equal_properties_from_distinct_arcs_share_one_palette_entry() {
        ...
        let out = section.to_section(0);
        let stone_entries = out
            .block_states
            .palette
            .iter()
        ...
        assert_eq!(
            stone_entries, 1,
            "equal compounds must not split the palette"
        );
    }

    /// The same Arc reused across a section must also resolve to one entry. This is the
    /// path the memo actually exists for: a field of crops shares one interned compound.
    #[test]
    ...
    #[test]
    fn full_and_fullwide_same_content_read_identically() {
        // The seam-merge content hash derives from get(0) + iter(); a Full(u8) section and a
        // FullWide section with the SAME logical content must read element-for-element identically,
        // or seam dedup/reconcile would silently corrupt blocks 256-265.
        let mut full = BlockStorage::Uniform(AIR);
        full.set(5, STONE); // stays Full(u8)
        assert!(matches!(full, BlockStorage::Full(_)));

        let mut wide = BlockStorage::Uniform(AIR);
        wide.set(5, KELP); // -> FullWide
        wide.set(5, STONE); // overwrite back to a narrow id; representation stays FullWide
        assert!(matches!(wide, BlockStorage::FullWide(_)));

        for i in 0..4096 {
            assert_eq!(full.get(i), wide.get(i), "mismatch at {i}");
        }
        assert!(
            full.iter().eq(wide.iter()),
            "iter() must agree across representations"
        );
    }
